...
const { GITHUB_API_TOKEN, REPO_NAME, PR_REVIEWERS, PR_TEAM_REVIEWERS } = process.env;
const REPO = `https://api.github.com/repos/ifit/${REPO_NAME}`;
const defaults = {
  headers: {
...
async function requestReview(pullNumber) {
  return POST(
    ...
    { 
      team_reviewers: (PR_TEAM_REVIEWERS || "").split(','),
      reviewers: (PR_REVIEWERS || "").split(','),
    }
  );
}
